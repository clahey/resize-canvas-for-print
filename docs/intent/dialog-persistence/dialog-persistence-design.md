...
| Unit widget, both groups | Plain `Gtk.SpinButton`s laid out by hand, plus a real `GimpUi.UnitComboBox`; the plugin drives conversion itself on the dropdown's `changed` signal | `GimpUi.SizeEntry` owning its own field(s) and unit menu, for one or both groups; a hidden `SizeEntry` field mirroring the last-edited axis, for print size specifically | `GimpSizeEntry` bundles its spin button(s) and unit dropdown into one composite widget with a fixed internal layout that didn't reproduce GIMP's own width-above-height-with-trailing-dropdown convention when tried, and offered no way to keep print size's aspect-lock/last-edited-axis bookkeeping working (a hidden mirror field would still fire `value-changed` events indistinguishable from other `SizeEntry`-driven ones). Driving plain spin buttons and a real unit dropdown by hand gives full control over layout and event handling, and keeps both groups visually and behaviorally identical. `GimpUi.UnitComboBox` itself has no confirmed way to exclude pixels/percent (RCFP-DIALOG-UNIT-006); an unrecognized selection falls back to inches rather than crashing, but pixels/percent may still be choosable — needs live-GIMP confirmation either way. |
